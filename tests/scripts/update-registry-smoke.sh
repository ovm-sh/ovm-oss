...
# ship zsh). Exercise the QM updater against a fixture npm so the test is
...
PATH="$mock_bin:$PATH" MOCK_NPM_SCENARIO=initial \
  bash "$fixture_repo/scripts/update-registry.sh" qm

for f in "$fixture_repo/docs/api/qm.json" "$fixture_repo/docs/api/registry.json"; do
...
api_dir = sys.argv[1]
with open(f"{api_dir}/qm.json") as f:
    qm = json.load(f)
assert qm["product"] == "qm", qm
assert [entry["version"] for entry in qm["versions"]] == [
    "0.1.0", "0.1.1", "0.1.2", "0.1.3", "0.1.4", "0.1.5", "0.1.6"
], qm
by_version = {entry["version"]: entry for entry in qm["versions"]}
...
cp "$fixture_repo/docs/api/qm.json" "$fixture_root/qm-before.json"
if PATH="$mock_bin:$PATH" MOCK_NPM_SCENARIO=reduced OVM_ALLOW_MASS_RETIREMENT=all \
  bash "$fixture_repo/scripts/update-registry.sh" qm >"$fixture_root/blanket.out" 2>&1; then
  echo "QM mass-retirement blanket override unexpectedly succeeded" >&2
  exit 1
fi
grep -q 'OVM_ALLOW_MASS_RETIREMENT=all permits nothing' "$fixture_root/blanket.out"
cmp "$fixture_root/qm-before.json" "$fixture_repo/docs/api/qm.json"
...
PATH="$mock_bin:$PATH" MOCK_NPM_SCENARIO=reduced OVM_ALLOW_MASS_RETIREMENT=qm \
  bash "$fixture_repo/scripts/update-registry.sh" qm

python3 - "$fixture_repo/docs/api/qm.json" <<'PY'
import json
import sys

with open(sys.argv[1]) as f:
    qm = json.load(f)
assert [entry["version"] for entry in qm["versions"]] == ["0.1.6"], qm
assert len(qm["retired_versions"]) == 6, qm
assert {entry["version"] for entry in qm["retired_versions"]} == {
    "0.1.0", "0.1.1", "0.1.2", "0.1.3", "0.1.4", "0.1.5"
}, qm
print("update-registry-smoke: ok (QM fixtures, metadata, retirement guard)")
PY
