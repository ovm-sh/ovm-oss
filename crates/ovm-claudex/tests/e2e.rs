...
fn seeded_home(temp: &Path, port: u16) {
    ...
    .unwrap();
    // Launch fails fast without a stored OAuth grant; seed one like setup does.
    let auth_dir = temp.join("proxy").join("auth");
    std::fs::create_dir_all(&auth_dir).unwrap();
    std::fs::write(auth_dir.join("e2e-grant.json"), "{}").unwrap();
}
...
fn explicit_update_migrates_a_legacy_daemon_after_sessions_are_closed() {
    let _serial = serialize_process_test();
    // Under a parallel full-suite run the freshly spawned proxy can take well
    // over the default 10s poll budget to get scheduled; the deadline guards
    // behavior, not scheduling, so give it slack (children inherit this).
    std::env::set_var("OVM_CLAUDEX_PROXY_START_ATTEMPTS", "120");
    let temp = tempfile::tempdir().unwrap();
    let proxy_port = unused_local_port();
    seeded_home(temp.path(), proxy_port);
    std::fs::create_dir_all(temp.path().join("proxy")).unwrap();
...
#[test]
fn launch_without_oauth_grant_fails_fast() {
    let temp = tempfile::tempdir().unwrap();
    seeded_home(temp.path(), 8317);
    // Remove the seeded grant: launch must refuse before touching the proxy
    // or spawning Claude — an ungranted proxy hangs every request (RC finding).
    std::fs::remove_dir_all(temp.path().join("proxy").join("auth")).unwrap();
