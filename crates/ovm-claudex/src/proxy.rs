...
/// Whether the proxy has any stored OAuth grant. Without one, no request can
/// ever authenticate — launching a session would hang on the first prompt, so
/// callers fail fast instead (doctor reports the same condition).
pub fn has_oauth_grant(dirs: &ClaudexDirs) -> bool {
    std::fs::read_dir(dirs.proxy_auth_dir())
        .map(|entries| entries.flatten().any(|entry| entry.path().is_file()))
        .unwrap_or(false)
}

/// Whether any launched claudex session still holds the shared session lease.
/// Used by `stop` to tell the operator which processes survive the proxy.
pub fn live_sessions_exist(dirs: &ClaudexDirs) -> bool {
...
pub enum ProbeIdentity<'a> {
    ...
    /// currently own the server side of the exact established connection that
    /// would carry the key. Every production caller uses this — the pidfile is
    /// written the instant claudex spawns the proxy, so it is present for both
    /// freshly-spawned and adopted daemons.
...
/// Whether the established connection could be positively tied to our proxy.
enum ListenerIdentity {
    /// Our verified proxy pid owns the server end — safe to send the key.
    Confirmed,
...
fn connect_models(port: u16) -> Option<TcpStream> {
    let addr = SocketAddrV4::new(Ipv4Addr::LOCALHOST, port);
    let stream = TcpStream::connect_timeout(&addr.into(), Duration::from_millis(300)).ok()?;
    let _ = stream.set_read_timeout(Some(Duration::from_secs(3)));
    let _ = stream.set_write_timeout(Some(Duration::from_secs(3)));
    Some(stream)
}

/// Raw localhost HTTP/1.1 GET of `/v1/models` on an already-established
/// connection (no TLS, no client dependency).
fn fetch_models_on(mut stream: TcpStream, port: u16, api_key: &str) -> (String, String) {
    let request = format!(
...
/// Connect and fetch in one step for non-credential-bearing calls.
fn fetch_models(port: u16, api_key: &str) -> Option<(String, String)> {
    let stream = connect_models(port)?;
    Some(fetch_models_on(stream, port, api_key))
}
...
/// 2. **Connected ownership**: canary rejection alone proves nothing (any
///    squatter can 401). We establish the connection that would carry the real
///    key, then demand positive identity via [`ProbeIdentity`]: the pidfile
///    must still identify our proxy and `lsof` must confirm that pid owns the
///    server side of this exact `ESTABLISHED` TCP flow.
/// 3. **Auth**: only once the already-connected socket is confirmed do we send
///    the real key on that same socket. A later listener exit/rebind cannot
///    redirect bytes from an established flow to the replacement process.
///
...
pub fn probe(port: u16, api_key: &str, identity: ProbeIdentity) -> ProxyProbe {
    ...
    // Establish the would-be authenticated connection before checking
    // ownership. The identity check below binds to this exact TCP flow, and the
    // key is sent on the same stream only after it succeeds.
    let Some(auth_stream) = connect_models(port) else {
        return ProxyProbe::Down;
    };
    match check_listener_identity(port, &auth_stream, &identity) {
    ...
    let (status, body) = fetch_models_on(auth_stream, port, api_key);
    match status.as_str() {
...
/// Confirm the established connection on `port` reaches our proxy before the
/// key is transmitted.
/// The pidfile identifies the pid, `pid_matches_record` proves that pid is
/// still our proxy process, and `lsof` proves that pid owns the server side of
/// this exact TCP flow.
fn check_listener_identity(
    port: u16,
    stream: &TcpStream,
    ...
    match pid_owns_connected_socket(record.pid, stream) {
        Ok(true) => ListenerIdentity::Confirmed,
        Ok(false) => ListenerIdentity::Foreign(format!(
            "pid {} does not own the established proxy connection on 127.0.0.1:{port}",
            record.pid
        )),
        // `lsof` missing or unusable: we cannot prove ownership, so we refuse
        // to send the key rather than trust the canary rejection.
...
/// Does `pid` own the server side of this exact established TCP flow? Checking
/// the accepted connection's four-tuple binds identity to the socket that will
/// carry the real key, rather than to a listening port that can be rebound
/// between verification and a later connection.
fn pid_owns_connected_socket(pid: u32, stream: &TcpStream) -> std::result::Result<bool, String> {
    let client = stream.local_addr().map_err(|error| {
        format!("could not identify the proxy connection's local end ({error})")
    })?;
    let server = stream
        .peer_addr()
        .map_err(|error| format!("could not identify the proxy connection's peer ({error})"))?;
    let expected_flow = format!("{server}->{client}");
    let output = lsof_command()
        ...
        .map_err(|error| {
            format!(
                "could not run `lsof` to confirm claudex's proxy owns the established connection ({error}); \
    ...
    Ok(stdout.lines().any(|line| {
        line.strip_prefix('n')
            .and_then(|flow| flow.split_whitespace().next())
...
mod tests {
    ...
    #[test]
    fn connected_socket_identity_is_bound_to_the_exact_tcp_flow() {
        let listener = std::net::TcpListener::bind("127.0.0.1:0").expect("bind");
        ...
        let server = std::thread::spawn(move || {
            ...
            release_rx.recv().expect("hold accepted connection");
        });

        let stream = TcpStream::connect(address).expect("connect");
        accepted_rx.recv().expect("wait for accept");

        assert!(pid_owns_connected_socket(std::process::id(), &stream).unwrap());
