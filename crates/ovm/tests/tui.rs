...
fn create_installed_codex_dir(home: &Path, version: &str) {
    let version_dir = home
        ...
        .join("versions")
        .join(version);
    fs::create_dir_all(&version_dir).expect("create installed codex version dir");
    fs::write(version_dir.join(".legacy-install"), b"archived")
        .expect("mark archived codex install");
}
