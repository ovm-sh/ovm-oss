...
fn failed_install_leaves_no_trace() {
    ...
    let _release = server
        ...
        .create();
    let _npm = server
        .mock("GET", Matcher::Regex(r"^/999\.999\.999-[a-z0-9-]+$".into()))
        .with_status(404)
        .expect(1)
        .create();
    ...
    ovm(home.path(), &releases_url)
        ...
        .failure()
        .stderr(predicates::str::contains(
            "Could not download Codex rust-v999.999.999",
        ));
    ...
    assert!(
        !version_dir.exists(),
        "failed install left version directory at {}",
        version_dir.display()
    );
    ...
    let list_output = ovm(home.path(), &releases_url)
        ...
        .stdout
        .clone();
    let list_output = String::from_utf8_lossy(&list_output);
    assert!(list_output.contains("No codex versions installed"));
    assert!(!list_output.contains(version));
}
