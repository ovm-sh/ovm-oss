...
fn run_git(cwd: &Path, args: &[&str]) -> Option<String> {
    let output = isolated_git_command(cwd, args).output().ok()?;
...
fn isolated_git_command(cwd: &Path, args: &[&str]) -> Command {
...
#[cfg(test)]
mod tests {
    use super::{isolated_git_command, DevInstallMetadata, DevInstallMode};
    use std::fs;
    use std::path::Path;
    ...
    fn git(repo: &Path, args: &[&str]) {
        ...
        let mut command = isolated_git_command(repo, args);
