...
/// The first release whose control plane ships the `ovm self` management
/// commands (`use`/`rollback`/`list`). Anything older cannot switch back once
/// it becomes the active control plane.
...
impl SelfManager {
    ...
    pub fn config_file(&self) -> &Path {
        &self.ovm_dirs.config_file
    }

    /// Whether `version`, once active, can switch back on its own. Releases
    /// before [`FIRST_SELF_MANAGED_RELEASE`] have no `ovm self` commands —
    /// activating one is a one-way door (recovery means rerunning the
    /// installer). Dev snapshots and unparseable ids are assumed capable.
    pub fn can_switch_back(version: &str) -> bool {
        ...
        let floor =
            semver::Version::parse(FIRST_SELF_MANAGED_RELEASE).expect("floor constant parses");
        parsed >= floor
    }
    ...
    pub fn rollback_to(&self, expected_previous: &str) -> Result<String> {
        ...
        if previous != expected_previous {
            return Err(OvmError::Message(format!(
                "OVM rollback target changed from {expected_previous} to {previous}; retry the command"
            )));
        }
    ...
    /// Remove inactive *release* versions whose install directory is older
    /// than `days` (by mtime). The current and previous versions are always
    /// kept, as are dev snapshots — those have their own keep-current-and-
    /// previous pruning in [`Self::prune_inactive_dev_versions`]. Backs the
    /// opt-in `self.retentionDays` setting; returns the removed versions.
    pub fn prune_releases_older_than(&self, days: u32) -> Result<Vec<String>> {
...
mod tests {
    ...
    fn switches_and_rolls_back_dynamic_side_sets() {
        ...
        assert_eq!(manager.rollback_to("dev-first").unwrap(), "dev-first");
        assert_eq!(
            manager.current_version().unwrap().as_deref(),
            Some("dev-first")
        );
    ...
    fn rollback_escapes_a_corrupt_current_manifest() {
        ...
        assert_eq!(manager.rollback_to("good").unwrap(), "good");
    ...
    #[test]
    fn switch_back_capability_uses_the_self_managed_floor() {
        // Pre-self-management releases are one-way doors.
        assert!(!SelfManager::can_switch_back("0.0.1"));
        assert!(!SelfManager::can_switch_back("0.0.2"));
        // The floor itself and everything newer can come back.
        assert!(SelfManager::can_switch_back("0.0.3-alpha.1"));
        assert!(SelfManager::can_switch_back("0.0.3"));
        assert!(SelfManager::can_switch_back("1.2.3"));
        // Dev snapshots and unparseable ids are assumed capable.
        assert!(SelfManager::can_switch_back("dev-anything"));
        assert!(SelfManager::can_switch_back("not-semver"));
    }

    #[test]
    fn retention_prunes_only_inactive_releases_past_cutoff() {
        let temp = tempdir().unwrap();
        let manager = manager(temp.path());
        let manifest = fixture_manifest(&[]);
        for version in ["0.1.0", "0.2.0", "0.3.0", "dev-old"] {
            let source = fixture_source(temp.path(), &manifest, version);
            manager.install_bundle(version, &manifest, &source).unwrap();
        }
        manager.use_version("0.2.0").unwrap();
        manager.use_version("0.3.0").unwrap();

        // A generous window keeps everything.
        assert!(manager
            .prune_releases_older_than(10_000)
            .unwrap()
            .is_empty());

        // A zero-day window prunes every inactive release immediately, but
        // never the current/previous selections or dev snapshots.
        assert_eq!(manager.prune_releases_older_than(0).unwrap(), vec!["0.1.0"]);
        assert!(manager.is_complete("0.2.0"));
        assert!(manager.is_complete("0.3.0"));
        assert!(manager.is_complete("dev-old"));
        assert!(!manager.is_complete("0.1.0"));
    }
