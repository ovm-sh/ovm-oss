...
/// (always listed; opens the self menu with settings + version swapping).
...
/// Build the product-picker rows paired with the pick they resolve to. OVM is
/// always listed — whether its menu offers version *swapping* is governed
/// inside the self menu (`self.manageVersions`). Kept pure so the list is
/// unit-testable without a terminal.
fn product_picker_entries() -> Vec<(String, ProductPick)> {
...
fn render_self_row(row: &SelfVersionRow, version_width: usize) -> String {
    ...
    } else if row.no_return {
        // One-way door: greyed out; the menu refuses to select it.
    ...
    let marker = if row.current {
        ...
        style("no way back").dim().to_string()
    } else {
        String::new()
    };
...
/// Live settings state for the OVM self menu. The picker mutates it in place
/// as toggles cycle; the caller persists it to config when `dirty` on exit.
pub(super) struct SelfMenuState {
    /// Explicit `self.manageVersions` setting; `None` inherits `manage_default`.
    pub manage: Option<bool>,
    /// What `None` means: on for the alpha channel / legacy flag, else off.
    pub manage_default: bool,
    pub auto_update: crate::config::AutoUpdatePolicy,
    pub retention_days: Option<u32>,
    pub dirty: bool,
}

impl SelfMenuState {
    pub fn manage_on(&self) -> bool {
        self.manage.unwrap_or(self.manage_default)
    }

    fn toggle_manage(&mut self) {
        self.manage = Some(!self.manage_on());
        self.dirty = true;
    }

    fn cycle_auto_update(&mut self) {
        use crate::config::AutoUpdatePolicy as Policy;
        self.auto_update = match self.auto_update {
            Policy::On => Policy::Notify,
            Policy::Notify => Policy::Off,
            Policy::Off => Policy::On,
        };
        self.dirty = true;
    }

    fn cycle_retention(&mut self) {
    ...
    fn manage_label(&self) -> String {
        let value = if self.manage_on() { "on" } else { "off" };
    ...
    fn retention_label(&self) -> String {
        match self.retention_days {
            None => "keep forever".into(),
            Some(days) => format!("{days} days"),
        }
    }
}

/// One row of the self menu: a cyclable setting or a switchable version.
...
/// The menu's row list for the current state: three settings, then the
/// version rows when managing is on. Pure for unit tests.
fn self_menu_rows(state: &SelfMenuState, version_count: usize) -> Vec<SelfMenuRow> {
    let mut menu = vec![
        SelfMenuRow::ManageToggle,
        SelfMenuRow::AutoUpdateToggle,
        SelfMenuRow::RetentionToggle,
    ];
    if state.manage_on() {
        menu.extend((0..version_count).map(SelfMenuRow::Version));
    }
    menu
}

fn render_self_setting(label: &str, value: &str) -> String {
    format!("{}  {}", style(format!("{label:<17}")).dim(), value)
}

/// The OVM entry's menu: a short "OVM manages OVM too" header, cyclable
/// settings (manage on/off, auto-update policy, retention), and — when
/// managing is on — the installed self versions. Enter on a setting cycles
/// it (as do ←/→); Enter on a version returns its index for the switch.
pub(super) fn pick_self_menu(
    state: &mut SelfMenuState,
    rows: &[SelfVersionRow],
    can_go_back: bool,
) -> Result<Option<usize>> {
    let term = Term::stderr();
    ...
    loop {
        let menu = self_menu_rows(state, rows.len());
        cursor = cursor.min(menu.len() - 1);

        let mut lines: Vec<String> = Vec::new();
        lines.push(String::new());
        lines.push(format!("  {}", style("OVM — Open Version Manager").bold()));
        lines.push(format!(
            "  {}",
            style("OVM can manage OVM versions too; leave it on to swap versions here.").dim()
        ));
        lines.push(String::new());

        for (i, row) in menu.iter().enumerate() {
            let rendered = match row {
                SelfMenuRow::ManageToggle => {
                    render_self_setting("manage versions", &state.manage_label())
                }
                SelfMenuRow::AutoUpdateToggle => {
                    render_self_setting("auto-update", state.auto_update.label())
                }
                SelfMenuRow::RetentionToggle => {
                    render_self_setting("old versions", &state.retention_label())
                }
                SelfMenuRow::Version(index) => render_self_row(&rows[*index], version_width),
            };
            // Blank separator between the settings block and the version list.
            if matches!(row, SelfMenuRow::Version(0)) {
                lines.push(String::new());
            }
            if i == cursor {
        ...
        if !state.manage_on() {
            ...
            lines.push(format!(
                "  {}",
                style("turn manage versions on to swap between installed OVM versions").dim()
            ));
        } else if rows.is_empty() {
            lines.push(String::new());
            lines.push(format!(
                "  {}",
                style("no self-managed versions installed yet — `ovm self update` adds them").dim()
            ));
        }

        if let Some(text) = &notice {
            lines.push(String::new());
            lines.push(format!("  {} {}", style("!").yellow().bold(), text));
        }

        lines.push(String::new());
        let back_label = if can_go_back { "back" } else { "quit" };
        let action_label = if matches!(menu[cursor], SelfMenuRow::Version(_)) {
            "select"
        } else {
            "toggle"
        };
        ...
        match key {
            ...
            Key::ArrowDown | Key::Char('j') if cursor + 1 < menu.len() => {
                cursor += 1;
                notice = None;
            }
            Key::ArrowLeft | Key::ArrowRight | Key::Char(' ') | Key::Enter => match menu[cursor] {
                SelfMenuRow::ManageToggle => state.toggle_manage(),
                SelfMenuRow::AutoUpdateToggle => state.cycle_auto_update(),
                SelfMenuRow::RetentionToggle => state.cycle_retention(),
                SelfMenuRow::Version(index) => {
                    if key == Key::Enter {
                        // One-way door: refuse and explain instead of switching.
                        if rows[index].no_return {
                            notice = Some(format!(
                                "{} predates self-management and could not switch back — \
                                 selection blocked",
                                rows[index].version
                            ));
                        } else {
...
mod tests {
    ...
    #[test]
    fn product_picker_always_lists_ovm_last() {
        let entries = product_picker_entries();
        ...
        let (ovm_label, pick) = entries.last().expect("entries");
        assert_eq!(*pick, ProductPick::Ovm, "OVM is always the final entry");
        let ovm_label = plain(ovm_label);
        assert!(ovm_label.contains("ovm"), "{ovm_label}");
        assert!(ovm_label.contains("Open Version Manager"), "{ovm_label}");
    }

    fn menu_state(manage: Option<bool>, manage_default: bool) -> SelfMenuState {
        SelfMenuState {
            manage,
            manage_default,
            auto_update: crate::config::AutoUpdatePolicy::On,
            retention_days: None,
            dirty: false,
        }
    }
    ...
    #[test]
    fn self_menu_hides_version_rows_until_managing_is_on() {
        let off = self_menu_rows(&menu_state(None, false), 3);
        assert_eq!(off.len(), 3, "settings only when managing is off");
        assert!(!off.iter().any(|row| matches!(row, SelfMenuRow::Version(_))));

        let on = self_menu_rows(&menu_state(None, true), 3);
        assert_eq!(on.len(), 6, "three settings + three versions");
        assert!(matches!(on[3], SelfMenuRow::Version(0)));
    }

    #[test]
    fn manage_toggle_becomes_explicit_and_overrides_the_default() {
        let mut state = menu_state(None, true);
        assert!(state.manage_on(), "inherits the alpha-channel default");
        state.toggle_manage();
        assert_eq!(state.manage, Some(false), "toggling pins an explicit value");
        assert!(!state.manage_on());
        assert!(state.dirty);
    }

    #[test]
    fn auto_update_and_retention_cycles_wrap() {
        use crate::config::AutoUpdatePolicy as Policy;
        let mut state = menu_state(None, false);
        state.cycle_auto_update();
        assert_eq!(state.auto_update, Policy::Notify);
        state.cycle_auto_update();
        assert_eq!(state.auto_update, Policy::Off);
        state.cycle_auto_update();
        assert_eq!(state.auto_update, Policy::On);

        state.cycle_retention();
        assert_eq!(state.retention_days, Some(30));
        state.cycle_retention();
        assert_eq!(state.retention_days, Some(90));
        state.cycle_retention();
        assert_eq!(state.retention_days, None, "cycle wraps back to forever");
        assert!(state.dirty);
    }

    #[test]
    fn self_menu_labels_mark_inherited_manage_values() {
        assert_eq!(menu_state(None, true).manage_label(), "on (default)");
        assert_eq!(menu_state(Some(true), false).manage_label(), "on");
        assert_eq!(menu_state(Some(false), true).manage_label(), "off");
        let mut state = menu_state(None, false);
        assert_eq!(state.retention_label(), "keep forever");
        state.retention_days = Some(30);
        assert_eq!(state.retention_label(), "30 days");
    }
    ...
    #[test]
    fn self_row_flags_one_way_versions() {
        ...
        let rendered = plain(&render_self_row(&row, 16));
        assert!(rendered.contains("no way back"), "{rendered}");
    }
