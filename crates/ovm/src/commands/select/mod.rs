...
/// literal `ovm` product is always accepted and opens the self menu (settings
/// plus version swapping when `self.manageVersions` allows it).
...
fn load_self_menu_config(path: &Path) -> Result<crate::config::OvmConfig> {
    crate::config::OvmConfig::load(path).map_err(|error| {
        OvmError::Message(format!(
            "Cannot open OVM self-management because the config at {} could not be loaded: \
             {error}. Fix the file or move it aside, then retry.",
            path.display()
        ))
    })
}

/// The OVM product menu: self-management settings (manage toggle, auto-update
/// policy, retention window) plus — when managing is on — the installed `ovm
/// self` versions (current + previous marked, dev snapshots tagged). Setting
/// changes persist to config on exit; selecting a version drives the exact
/// `ovm self use` path (atomic, next-command activation, rollback-safe).
fn run_self_version_picker(can_go_back: bool) -> Result<PickerResult> {
    ...
    let mut state = picker::SelfMenuState {
        manage: config.self_.manage_versions,
        manage_default: config.self_manage_default(),
        auto_update: config.self_.auto_update,
        retention_days: config.self_.retention_days,
        dirty: false,
    };
    // The menu's live derivation must agree with the config's canonical one.
    debug_assert_eq!(state.manage_on(), config.self_versions_manageable());
    let picked = picker::pick_self_menu(&mut state, &rows, can_go_back)?;

    // Persist toggles on any exit path — esc after toggling still counts.
    if state.dirty {
        if config.self_.manage_versions != state.manage {
            config.set_self_manage_versions(state.manage);
        }
        if config.self_.auto_update != state.auto_update {
            config.set_self_auto_update(state.auto_update);
        }
        if config.self_.retention_days != state.retention_days {
            config.set_self_retention_days(state.retention_days);
        }
...
#[cfg(test)]
mod tests {
    use super::{fetch_release_dates, load_self_menu_config, mark_uninstalled, VersionEntry};
    use crate::product::Product;
    use mockito::Server;
    ...
    #[test]
    fn self_menu_config_defaults_only_when_missing() {
        let temp = tempfile::tempdir().unwrap();
        let path = temp.path().join("config.json");

        assert!(load_self_menu_config(&path).is_ok());

        let malformed = b"{\"self\":";
        std::fs::write(&path, malformed).unwrap();
        let error = load_self_menu_config(&path).unwrap_err();
