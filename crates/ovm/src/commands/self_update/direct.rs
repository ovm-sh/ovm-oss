...
fn update_with(
    ...
    if result.is_ok() {
        apply_retention(manager, &operation);
    }
    drop(operation);
    result?;
    eprintln!(
...
/// Opt-in retention (`self.retentionDays`): after a successful update
/// activation, drop inactive release versions older than the window. Fail-open
/// on every path — retention must never break an update that already
/// succeeded, so config or prune errors are ignored.
pub(crate) fn apply_retention(
    manager: &SelfManager,
    _operation: &crate::self_manager::SelfOperationLock,
) {
    let Ok(config) = crate::config::OvmConfig::load(manager.config_file()) else {
        return;
    };
    let Some(days) = config.self_.retention_days else {
        return;
    };
    let Ok(removed) = manager.prune_releases_older_than(days) else {
        return;
    };
    for version in removed {
        eprintln!(
            "  {}",
            console::style(format!("pruned OVM {version} (retention {days}d)")).dim()
        );
    }
}
...
mod tests {
    ...
    fn release_repository_check_uses_path_not_substring() {
        ...
        assert!(require_release_repository(
            "https://github.com/attacker/evil/releases/download/v9.9.9/ovm.tar.gz?x=/ovm-sh/ovm-oss/releases/",
        )
        .is_err());
    ...
    // Under a parallel full-suite run the probe's spawned child can take well
    // over the default 10s to get scheduled; the deadline guards behavior, not
    // scheduling. Broken-probe tests are unaffected (their child exits fast).
    fn patient_activation_probe() {
        std::env::set_var("OVM_SELF_UPDATE_PROBE_TIMEOUT_MS", "60000");
    }

    #[test]
    fn self_update_installs_and_activates_a_release() {
        patient_activation_probe();
        let temp = tempdir().unwrap();
