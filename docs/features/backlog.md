...
| Launch-time prune must not silently destroy | **Data loss, 0.1.0 blocker.** Every bare launch calls `prune_all_products` (`commands/launch.rs`), which *uninstalls* — not archives — every inactive, non-`dev:`, non-archived version older than `cleanup.retention`. On 2026-08-01 typing `ccy` deleted **209 installs / 32.4 GB** with no prompt, no cap, no dry run and no undo; the message read `Cleaned up 209 old install(s)`, which sounds like cache. Archived count stayed 0 and the artifact cache was 100K, so nothing was locally recoverable. Fix: keep silent auto-prune only below a small threshold, and above it either require confirmation or defer to an explicit `ovm cleanup run`; prefer archiving to deletion; say what will go BEFORE removing it | **blocker** |
| Truthful `clean --all` wording | `ovm clean qm --all` printed `✓ Cleaned all qm versions, freed 0 B` while removing only cached artifacts and leaving the installed version — the command is doing its documented job (cache removal) but the message claims version removal, the same absence-rendered-as-a-verdict shape the devlogs keep cataloguing. Say what was actually cleaned ("cached artifacts for N versions") | planned |
| A way to fully leave a product | `uninstall` refuses the active version and `use` demands another installed version to switch to, so removing a product's only install requires `rm -rf ~/.ovm/products/<p>` by hand (hit for real during the 2026-08-08 QM testing). Add `ovm uninstall <p> --all` (or let uninstalling the last version clear the selection) with a confirmation | planned |
