...
The npm package, Homebrew tap, and crates.io publication paths are prepared but
have not been published. Do not use package-manager install instructions yet:
`@mochiexists/ovm` is unpublished, `ovm-sh/homebrew-ovm` is private, and a
complete crates.io bundle installation has not been proven. Those channels are
optional follow-ups after the public GitHub release and installer pass clean
...
ovm self update --channel beta  # one-shot prerelease lane
ovm self current
ovm self list
...
version switching and rollback. Homebrew and Cargo update implementations are
prepared for their future package channels, but are not supported installation
paths until those packages are published. A direct update probes the new control
plane and restores both control and active version if activation fails. The
emergency recovery command is:
