...
if [ $# -eq 0 ]; then
    PRODUCTS=(claude codex pi qm cliproxyapi)
else
    PRODUCTS=("$@")
fi
...
# QM is npm-sourced, so the same rule as claude applies: npm enforces real
# semver on publish, and anything reaching here that is not semver is an error
# body wearing a list rather than a version list.
NPM_SEMVER_RE = re.compile(
    r'^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)'
    r'(?:-((?:0|[1-9][0-9]*|[0-9]*[A-Za-z-][0-9A-Za-z-]*)'
    r'(?:\.(?:0|[1-9][0-9]*|[0-9]*[A-Za-z-][0-9A-Za-z-]*))*))?'
...
def leading_calendar_date(value):
    # npm's time map is metadata, not authority. Keep a date only when the
    # value is a string beginning with a real ISO calendar date; all other
    # values are safely represented by the registry's already-optional field.
    try:
        return datetime.date.fromisoformat(value[:10]).isoformat()
    except (ValueError, TypeError):
        return None
...
def parse_semver(version):
    m = NPM_SEMVER_RE.match(version)
    if not m:
        return (0, 0, 0, 1, [])
    major, minor, patch, prerelease, _ = m.groups()
    core = (int(major), int(minor), int(patch))
    if not prerelease:
        return core + (1, [])
    key = [(0, int(part)) if part.isdigit() else (1, part) for part in prerelease.split('.')]
    return core + (0, key)
...
def compute_retired_versions(previous, current_versions):
    if not previous:
        return []
    current = {entry['version'] for entry in current_versions}
    previous_versions = {entry['version']: entry for entry in previous.get('versions', [])}
    retired = {
        entry['version']: entry
        for entry in previous.get('retired_versions', [])
        if entry.get('version') not in current
    ...
    guard_mass_retirement('qm', previous_versions, newly_retired)
    return sorted(retired.values(), key=lambda entry: parse_semver(entry['version']))

def npm_view(field):
    result = subprocess.run(
        ['npm', 'view', '@yc-software/qm', field, '--json'],
        capture_output=True, text=True, timeout=30,
    )
    if result.returncode != 0:
        raise SystemExit(f'npm {field} lookup failed: {result.stderr}')
    return json.loads(result.stdout)

raw_versions = npm_view('versions')
# A single-version package answers with a bare string, not a list.
if isinstance(raw_versions, str):
    raw_versions = [raw_versions]
if not isinstance(raw_versions, list) or not raw_versions:
    raise SystemExit(
        f'npm versions lookup: expected a non-empty list, got {raw_versions!r:.200}; '
        f'refusing to overwrite registry'
    )
bad = [v for v in raw_versions if not isinstance(v, str) or not NPM_SEMVER_RE.match(v)]
if bad:
    raise SystemExit(
        f'npm versions lookup returned non-semver entries {bad!r:.200}; '
        f'refusing to overwrite registry'
    )

time_data = npm_view('time')
if not isinstance(time_data, dict):
    raise SystemExit(
        f'npm time lookup: expected a JSON object, got {type(time_data).__name__}; '
        f'refusing to overwrite registry'
...
versions = []
for version in sorted(raw_versions, key=parse_semver):
    entry = {'version': version}
    published_at = leading_calendar_date(time_data.get(version))
    if published_at is not None:
        entry['date'] = published_at
    versions.append(entry)

dist_tags = npm_view('dist-tags')
if not isinstance(dist_tags, dict):
    raise SystemExit(
        f'npm dist-tags lookup: expected a JSON object, got {type(dist_tags).__name__}; '
        f'refusing to overwrite registry'
    )
published = {entry['version'] for entry in versions}
# Absence of a latest tag naming a published version means the answer was not a
# dist-tags document; publishing it would leave clients resolving 'qm latest'
...
dist_tags = {tag: (target if target in published else '') for tag, target in dist_tags.items()}

path = '$API_DIR/qm.json'
previous = load_previous(path)
retired_versions = compute_retired_versions(previous, versions)
registry = {
    'product': 'qm',
    'display_name': 'QM',
    'source': 'npm:@yc-software/qm',
    'updated_at': '$TIMESTAMP',
    'dist_tags': dist_tags,
    'versions': versions,
}
...
print(f'  {len(registry.get(\"versions\", []))} qm versions written')
"
}
